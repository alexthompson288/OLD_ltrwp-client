...
public class JSONInStream
{
	...
	public JSONInStream ContentOptional(string tag, ref float value)
	{
		JSONNumberFieldValue fieldValue = (JSONNumberFieldValue) node.GetField(tag);
		if(fieldValue != null)
			value = (float) fieldValue.value;
	...
	{
		JSONBooleanFieldValue fieldValue = (JSONBooleanFieldValue) node.GetField(tag);
		value = fieldValue.value;
		return this;
	}
	...
	{
		JSONBooleanFieldValue fieldValue = (JSONBooleanFieldValue) node.GetField(idx);
		value = fieldValue.value;
		return this;
	}
  ...
  //---------------------------------------------------------------------------------
	...
	{
		JSONListFieldValue fieldValue = (JSONListFieldValue) node.GetField(tag);
		int i = 0;
